...
public class 股票买卖5 {
     ...
     * f[i,0]=max(  f[i-1,0]  f[i-1,2]-w[i] )手中无货的第二天可以买上,或者不买
     * f[i,1]=f[i-1,0]+w[i] 把昨天的卖出去
     * f[i,2]=max(f[i-1][1],f[i-1,2]) 手中无货,可能昨天是手中无货的第一天,也可能大于等于2天,
     */
    static void time() {
        dp[0][0] = dp[0][1] = (int) -1e9;//非法方案,所以第一天定义为手中无货的第二天
        for (int i = 1; i <= n; i++) {
