 ...
 * 输出样例：
 * 7
 * 状态定义:f[i],表示所有以第i个元素结尾子序列,并且不超过m个元素的最大和
 * 属性max
 * 假设以A[k]结尾,往前枚举,长度1的,长度2的,长度3...最大长度m
 * 结合区间和数组为S[],那么以A[k]结尾,长度为j的子序列和为,
 * S[k]-S[k-j](1<=j<=m)
 * 可以使用单调队列优化
 * 使得S[k-j]最小值,长度为m的序列中找到一个最小值
 * <p>
 * 只能用单调队列优化,其他都会超时,
 * 有限集合求最值,求个数
 * 考虑last
...
public class 最大序列和 {
    ...
    static void f() {
        ...
        //因为S[k]-S[k-j](1<=j<=m), 存在k<=m 也就是,结果S[k]-0也算是一种合法选择,然而取max为s[i]-s[q[hh]],
        //这么写最简单
        for (int i = 1; i <= n; i++) {
            if (q[hh] < i - m) hh++;
